<template>
                ...
                <DialogPanel class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-colorBgLight dark:bg-colorBgDark py-4 pb-6 shadow-xl">
                  ...
                  <div class="mt-4">
                    ...
                    <div class="border-t border-gray-200 pb-4 pt-4">
                      ...
                      <button @click="resetFilters" class="ml-4 mb-6 px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75">
              Clear Filters  
              </button>
                ...
                <div class="px-4 pb-2 pt-2" id="filter-section-0">
                  <div class="space-y-2">
                    <div class="flex items-center">
                      <input id="color-0-mobile" name="inventory" value="" type="radio" v-model="minimumStock" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500" >
                        ...
                        <DisclosurePanel class="px-4 pb-2 ">
                          ...
                          <div v-for="section in categories" :key="section.name" class="space-y-6">
                            <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center mt-2.5">
                              <input :id="`${section.id}-${optionIdx}-mobile`" name="category" :value="option.label" v-model="selectedCategory" type="radio" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                      ...
                      <fieldset>
                        ...
                        <DisclosurePanel class="px-4 pb-2">
                          ...
                          <div v-for="section in filters" :key="section.name" class="space-y-6">
                            <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center mt-2.5">
                              <input :id="`${section.id}-${optionIdx}-mobile`" name="variation" :value="option.label" v-model="selectedVariation" type="radio" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                              <label :for="`${section.id}-${optionIdx}-mobile`" class="ml-3 text-sm text-gray-500 dark:text-white">{{ option.label }}</label>
                            </div>
                          </div>
                        </DisclosurePanel>
        ...
        <main class="mx-auto max-w-2xl px-4 lg:max-w-7xl lg:px-8">
          ...
          <div class="pb-24 pt-12 lg:grid lg:grid-cols-3 lg:gap-x-8 xl:grid-cols-4">
            <aside>
              ...
              <button @click="resetFilters" class="hidden lg:block mb-6 px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75">
              Clear Filters  
              </button>
  ...
  <div v-for="category in categories" :key="category.id" >
    <template v-if="filteredProducts.some(product => product.category.includes(category.name))">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white underline mb-6 uppercase">{{ category.name }} <span v-if="selectedVariation != ''"> / {{ selectedVariation }}</span></h2>

      <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-3 lg:gap-x-8 mb-10">
        <div v-for="product in filteredProducts.filter(p => p.category.includes(category.name))" :key="product.id" class="mt-6">
          <NuxtLink :to="localePath('/product/' + product.id )">
            ...
            <div class="mt-4 flex justify-between">
              <div>
                <h3 class="text-lg font-extrabold text-gray-700 line-clamp-1 dark:text-white">{{ product.title }}</h3>
            ...
            <div class="basis-full flex flex-col ">
              <div class="text-lg font-medium text-gray-900 dark:text-white w-full basis-full">
                <p v-if="filtersList == 'Bitcoin'" class="float-left dark:text-white font-semibold text-black">
                  {{ (product.fiat * btcprices).toFixed(8) }} <BitcoinIcon class="h-6 w-6 inline" aria-hidden="true" />
                </p>
                <p v-if="filtersList == 'Sats'" class="float-left dark:text-white font-semibold text-black">
                  {{ (product.fiat * btcprices * 100000000).toFixed(0) }} <SatoshiV2Icon class="h-6 w-6 inline" aria-hidden="true" />
                </p>
              ...
              <div class="w-full dark:text-white basis-full"> 
                <span v-if="product.stock > 5 && product.stock != 0" class="flex items-center text-sm font-medium text-gray-900 dark:text-white">
                  <span class="flex w-2.5 h-2.5 bg-green-400 rounded-full mr-1.5 flex-shrink-0"></span>{{ t("Instock") }}
                </span>
                <span v-if="product.stock < 5 && product.stock > 0" class="flex items-center text-sm font-medium text-gray-900 dark:text-white">
