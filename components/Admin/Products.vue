...
<template>
  <div class="bg-colorBgLight dark:bg-colorBgDark">
    <div class="mx-auto max-w-2xl lg:max-w-4xl">
      <div
        class="mt-8 grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-2 xl:gap-x-8"
      >
        <div v-for="product in products" :key="product.id">
          ...
          <NuxtLink
            :to="localePath('/admin/product/edit')"
            class="float-right mt-2 w-3/5 snipcart-add-item hover:shadow-lg font-semibold py-2 px-4 rounded shadow max-h-10 text-black dark:text-white dark:hover:text-black hover:text-white bg-white hover:bg-black dark:bg-gray-700 dark:hover:bg-white"
          >
            <PencilSquareIcon
              class="-ml-0.5 h-5 w-5 text-gray-400 inline"
              aria-hidden="true"
            />
          ...
          <button
            class="mt-2 w-1/5 snipcart-add-item hover:shadow-lg font-semibold py-2 px-4 rounded shadow max-h-10 text-black dark:text-white dark:hover:text-black hover:text-white bg-white hover:bg-black dark:bg-gray-700 dark:hover:bg-white"
            @click="
              addToCart({
                id: product.id,
                amount: 1,
